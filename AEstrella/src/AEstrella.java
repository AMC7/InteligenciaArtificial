...
public class AEstrella extends PApplet {
    ...
    class Mosaico{
        ...
        public void modificaFamilia(){
        situacion=Situacion.EN_SOLUCION;
        Mosaico auxiliar=padre;
            while (auxiliar!=null) {
                auxiliar.situacion=Situacion.EN_SOLUCION;
        ...
        void calculaHeuristica(Mosaico meta){
            ...
            */
            hn =(abs(meta.columna-columna)+abs(meta.renglon-renglon))*10;
        }
    ...
    class NodoBusqueda implements Comparable<NodoBusqueda> {
        ...
        return "("+estado.columna+","+estado.renglon+")";
        }
    ...
    class Algoritmo {
        ...
        /**Método que inicializa todo aquello que estamos utilizando*/
        ...
        void inicializa(Mosaico estadoInicial, Mosaico estadoFinal) {
            ...
            nodoActual=new NodoBusqueda(estadoInicial);
            ...
            //Inicializazmos el estado final
            estadoInicial.calculaHeuristica(estadoFinal);
            estadoInicial.tipo = Tipo.ESTADO_INICIAL;
            //Añado el nodo a la lista abierta
            nodoPrevio =new NodoBusqueda(estadoInicial);
            listaAbierta.offer(nodoPrevio); 
            //procesaLosCuadrosAlcanzables
            procesaCuadrosAlcanzables(nodoPrevio);
        }
        
        /*Expande el siguiente nodo de la lista*/
        void expandeNodoSiguiente() {
            /**
              ...
              */
            if(!resuelto){
                //Paso 1 Lo saco de la lista abierta
                NodoBusqueda nodo=listaAbierta.poll();
                //Marco al que antes era el nodoActual como el nodo procesado
                nodoActual.estado.situacion=Situacion.EN_LISTA_CERRADA;
                //Lo que hago aqui es procesar los alcanzables modificando el padre si se es preciso
                procesaCuadrosAlcanzables(nodo); 
                //Se asigna como nodo actual al nodo con el cual estamos trabajando
                nodoActual=nodo;
                nodoActual.estado.situacion=Situacion.ACTUAL;
                nodoPrevio=nodoActual.padre;
                //Si esque ya llegamos al estado final trazamos el camino marcando a su padre 
                //como Nodo en solución
                if(nodo.estado.columna==estadoFinal.columna&&nodo.estado.renglon==estadoFinal.renglon){
                resuelto=true;
                nodo.estado.modificaFamilia();
                }
            }
        }
        /**Te dice si un nodo de busqueda está disponible para que me pueda 
         mover al mismo
         @param a El nodo que se quiere saber si esta disponible
         */
        private boolean nodoEstaDisponible(NodoBusqueda a){
        return 
               //1)Checa que no este en la lista cerrada 
               !listaCerrada.contains(a.estado)&&
               //Checa que pase a un estado disponible ya sea vacio o final 
               (a.estado.tipo.equals(Tipo.VACIO)||a.estado.tipo.equals(Tipo.ESTADO_FINAL));
        }
        
      /**Lo que hace es recibir un nodo y hace las operaciones pertinetes
       con los nodos alcanzables
       */
        private void procesaCuadrosAlcanzables(NodoBusqueda nodoPrevio) {
             LinkedList<NodoBusqueda> auxiliar=nodoPrevio.getSucesores();
                   for(NodoBusqueda vecino:auxiliar){
                      if(nodoEstaDisponible(vecino)){
                        if(vecino.estado.situacion.equals(Situacion.EN_LISTA_ABIERTA)){
                          if(vecino.gn<vecino.estado.gn){
                            this.nodoPrevio=nodoPrevio;
                            vecino.padre=nodoPrevio; 
                            vecino.estado.padre=vecino.padre.estado;
                            vecino.estado.calculaHeuristica(estadoFinal);
                            listaAbierta.remove(vecino);
                            listaAbierta.add(vecino);
                          }
                        }else{
                            vecino.padre=nodoPrevio;
                            vecino.estado.padre=vecino.padre.estado;    
                            vecino.estado.situacion=Situacion.EN_LISTA_ABIERTA;
                            vecino.estado.gn=vecino.gn;
                            vecino.estado.calculaHeuristica(estadoFinal);
                            listaAbierta.offer(vecino);
                        }
                      }
                   }     
              listaAbierta.remove(nodoPrevio);
